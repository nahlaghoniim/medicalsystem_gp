...
<div class="p-6 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto">
        ...
        <div class="bg-white rounded-2xl shadow p-6 mb-8">
            ...
            @if ($patient->prescriptions->isNotEmpty())
                ...
                <div class="flex gap-4 mt-6">
                    ...
                    <a href="{{ route('dashboard.doctor.patients.prescriptions.qr', [
                        'patient' => $patient->id,
                        'prescription' => $latestPrescription->id
                    ]) }}"
        ...
        <!-- AI Ray Diagnosis Section -->
        <div class="bg-white rounded-2xl shadow p-6 mb-8">
            <h3 class="text-xl font-bold text-[#1D5E86] mb-4">Upload Ray & Get AI Diagnosis</h3>

            @if(session('success'))
                <div class="bg-green-100 text-green-800 p-3 rounded mb-4">{{ session('success') }}</div>
            @endif

            <form action="{{ route('dashboard.doctor.patients.rays.store', $patient->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <input type="file" name="image" required class="mb-4 block">
                <button type="submit" class="bg-[#1D5E86] text-white px-6 py-2 rounded-lg hover:bg-blue-700 transition">
                    Analyze Ray
                </button>
            </form>

            @if($patient->rays->isNotEmpty())
                <div class="mt-6">
                    <h4 class="text-lg font-semibold text-gray-700 mb-2">Previous Ray Analyses</h4>
                    <ul class="space-y-2">
                        @foreach($patient->rays as $ray)
                            <li class="border p-4 rounded flex items-center gap-4">
                                <img src="{{ asset('storage/' . $ray->image) }}" class="w-20 h-20 rounded object-cover border" />
                                <div>
                                    <p class="text-sm text-gray-600"><strong>AI Diagnosis:</strong> {{ $ray->ai_diagnosis }}</p>
                                    <p class="text-xs text-gray-400">Uploaded on: {{ $ray->created_at->format('d M Y') }}</p>
                                </div>
                                 <!-- Delete Button -->
        <form action="{{ route('dashboard.doctor.patients.rays.destroy', ['patient' => $patient->id, 'ray' => $ray->id]) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this ray?');">
            @csrf
            @method('DELETE')
